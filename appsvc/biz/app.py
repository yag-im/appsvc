...
def age_mode_filter_expr(age_mode: AgeMode) -> ColumnElement[bool]:
    if age_mode == AgeMode.KID:
        return (AppDAO.esrb_rating < ESRB_RATING_T_ID) | (
            AppDAO.genres.contains([GENRE_EDUCATIONAL_ID]) | (AppDAO.genres.contains([GENRE_PUZZLE_ID]))
        )
...
def run_app(req: RunAppRequestDTO) -> RunAppResponseDTO:
    ...
    return RunAppResponseDTO(
        container=ContainerDescr(
            ...
            cpuset_cpus=run_container_res.container.cpuset_cpus,
        ),
    )
...
def search_by_publisher(req: SearchAppsRequestDTO, order_by: list) -> list[AppReleaseDAO]:
    ...
    publisher_exists = exists(
        select(1)
        .select_from(elem)
        .join(AppCompanyDAO, AppCompanyDAO.id == cast(elem_company_id_txt, Integer))
        .where(elem_is_publisher_txt == "true")
